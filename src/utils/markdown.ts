...
import type { BasicFieldType, BasicInfo, CustomItem, MenuSection, ResumeData } from '@/types/resume'
...
type ExportableBasicFieldKey = 'name' | 'title' | 'employementStatus' | 'birthDate' | 'email' | 'phone' | 'location'

const BASIC_FIELD_KEYS = new Set<ExportableBasicFieldKey>(['name', 'title', 'employementStatus', 'birthDate', 'email', 'phone', 'location'])
...
const pickBasicFieldValue = (basic: BasicInfo, key: ExportableBasicFieldKey) => normalizeText((basic[key] as string | undefined) || '')
...
const renderBasicSection = (title: string, resume: ResumeData, options?: ResumeMarkdownOptions) => {
...
const renderProjectSection = (title: string, resume: ResumeData) => {
  const blocks = resume.projects
    ...
    .map((item) => {
      ...
      const meta = [normalizeText(item.role), normalizeText(item.date)].filter(Boolean).join(' | ')
...
const renderEducationSection = (title: string, resume: ResumeData) => {
  const blocks = resume.education
    ...
    .map((item) => {
      ...
      const metadata = [normalizeText(item.degree), duration, item.gpa ? `GPA: ${normalizeText(item.gpa)}` : ''].filter(Boolean).join(' | ')
      const description = markdownFromText(item.description)
      const lines: string[] = []
...
export const generateResumeMarkdown = (resume: ResumeData, options?: ResumeMarkdownOptions) => {
